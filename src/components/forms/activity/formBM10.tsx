...
const FormBM10: FC<FormBM10Props> = ({
  onSubmit,
  initialData,
  mode,
  formName,
  ...
  const [eventsOrganizer, setEventsOrganizer] = useState<string>("");
    ...
    onDrop: useMemo(
      () => async (acceptedFiles: File[]) => {
        ...
        if (results && results !== undefined) {
          ...
          const response = await ImportCharitables(formDataImport);
  ...
  useEffect(() => {
    const loadUsers = async () => {
      ...
      if (mode === "edit" && initialData !== undefined) {
        ...
        setEventsOrganizer(initialData.eventsOrganizer[0]);
    ...
    <div className="grid grid-cols-1 mb-2">
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-2 gap-6 mb-2">
          <div className="flex flex-col gap-1">
            <span className="font-medium text-neutral-600">Đơn vị tổ chức</span>
            <TextArea
              autoSize
              value={eventsOrganizer}
              onChange={(e) => setEventsOrganizer(e.target.value)}
            />
          </div>
          <div className="flex flex-col gap-1">
