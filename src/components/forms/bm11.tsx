...
import FormBM11 from "./activity/formBM11";
...
const BM11 = () => {
  ...
  const columns: TableColumnsType<SchoolLevelItem> = [
    {
      title: "HOẠT ĐỘNG",
      dataIndex: "contents",
      key: "contents",
      className: "max-w-8",
      fixed: "left",
      render: (contents: string, record: SchoolLevelItem) => {
        return (
          <span
  ...
  const onSearch: SearchProps["onSearch"] = (value) => {
    if (value === "") setData(schoolLevels || []);

    const filteredData = schoolLevels.filter((item) => {
  ...
  const handleDelete = useCallback(async () => {
    try {
      ...
      if (selectedKeysArray.length > 0) {
        await deleteSchoolLevels(selectedKeysArray);
  ...
  const handleSubmit = async (formData: Partial<any>) => {
    try {
      if (mode === "edit" && selectedItem) {
        const response = await putSchoolLevel(formData.id as string, formData);
        if (response) {
          setFormNotification((prev) => ({
            ...prev,
            description: Messages.UPDATE_SCHOOLS_LEVEL,
          }));
        }
      } else {
        const response = await postSchoolLevel(formData);
        if (response) {
          setFormNotification((prev) => ({
            ...prev,
            description: Messages.ADD_SCHOOLS_LEVEL,
          }));
        }
      }
      setFormNotification((prev) => ({
  ...
  const handleExportExcel = async () => {
    setLoadingUpload(true);
    const results = await getExportSchoolLevel(selectedKey.id, null);
    ...
    const defaultInfo = [
      ...
      ["(ĐƠN VỊ)", "", "", ""],
      ["TỔNG HỢP DANH SÁCH"],
      ["Tham gia các hoạt động cấp Trường (ngày lễ, hoạt động lớn, ...)"],
      [""],
    ];
    ...
    let filename = "BM11-" + formattedDate + ".xlsx";
  ...
  useEffect(() => {
    setLoading(true);
    document.title = PageTitles.BM11;
    getDefaultYears();
    getDisplayRole();
    onSearch("");
    const timeoutId = setTimeout(() => {
  ...
  useEffect(() => {
    if (schoolLevels.length && (startDate || endDate)) {
      onSearch("");
    }
  }, [schoolLevels, startDate, endDate]);
  ...
  return (
    <div>
      ...
      <CustomModal
        ...
        title={
          mode === "edit"
            ? Messages.TITLE_UPDATE_SCHOOLS_LEVEL
            : Messages.TITLE_ADD_SCHOOLS_LEVEL
        }
        onOk={() => {
